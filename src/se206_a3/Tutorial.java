...
public class Tutorial {
    ...
    private void playVideo() throws IOException{
		...
		if (selectedFile.exists() ){
			...
			//"ffmpeg -i " + videoFile.getName() +  " 2>&1 | grep "Duration"
			mediaPlayerComponent.getMediaPlayer().mute(false);
			stdoutBuffered = CallBash.callBashBuffer("ffmpeg -i " + videoFile.getName() +  " 2>&1 | grep \"Duration\"");
			time.setText(Integer.toString(Test.runtimeToSecond(stdoutBuffered.readLine())));
			enableButtons();
		}
